...
<body>
	...
	<script type="text/javascript">
		...
		function changeHTML() {
			...
			if (xhr.readyState == 4) {
				// XMLHttpRequest.status : 웹프로그램 요청에 대한 응답의 상태코드(StatusCode)
				if (xhr.status == 200) { // 웹프로그램 요청에 대한 정상적인 응답결과를 받은 경우
					// XMLHttpRequest.responseText : 웹프로그램 요청에 대한 실행결과를 TEXT 또는
					// HTML로 응답받은 경우 응답결과를 저장한 프로퍼티
					document.getElementById("display").innerHTML = xhr.responseText;
				} else { // 웹프로그램 요청에 대한 비정상적인 응답결과를 받은 경우 - 에러코드(4XX 또는 5XX)
					alert("에러코드 = " + xhr.status);
				}
			} else {
				document.getElementById("display").innerHTML = "<image src = 'images/loading.gif' width = 50>"
			}
		}
